...
Durante il secondo periodo, il nostro team ha impegnato risorse per proseguire e quasi completare la definizione delle norme nel documento "Norme di progetto".

Nel corso del colloquio SAL con la proponente, è stato stabilito l'obiettivo di concludere entro la fine del secondo sprint la fase relativa al Proof of Concept (POC), focalizzata sull'integrazione dell'ultimo elemento nello stack tecnologico, "Grafana", e sulla visualizzazione di grafici delle misurazioni dei simulatori sviluppati.
...
Al termine del primo periodo, l'ammontare parziale totale del costo del progetto è \textbf{ 1563\euro\ } e sono state completate il \textbf{100\%} delle attività attese.
...
\subsubsection*{Preventivo e consuntivo orario per membro}
Nel corso del secondo periodo, sono state impiegate ore per l'esecuzione di attività interne, come dettagliato nella tabella precedente. Tali ore non sono state incluse nei costi e per svolgerle è intervenuto il ruolo di amministratore.
Per le attività registrate nei costi, sono stati assegnati i seguenti ruoli:

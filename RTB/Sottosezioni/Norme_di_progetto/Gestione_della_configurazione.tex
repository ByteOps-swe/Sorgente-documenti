...
Lo standard IEEE 828-2012\todo{link} definisce la gestione della configurazione del software come “un processo disciplinato per gestire l’evoluzione del software”.
...
L'incremento dei valori più significativi porta i meno significativi a zero.\todo{fornire un esempio} \\
Ogni variazione di versione deve essere presente nel registro delle modifiche.

\subsubsection{Repository}
Le repository\todo{è plulare, sarebbe meglio scrivere le repositories?} del team sono:
\begin{itemize}
...
Per approfondimenti su come creare issues e associare labels a una issue, consulta la sezione relativa al \hyperlink{par:ticketing}{Ticketing}.

Ad esempio, l'aggiunta di un elemento di configurazione al ramo principale (main) lo designa come la versione più recente e revisionata, mentre le versioni (X.0.0) con X>=1 indicano l'appartenenza del suddetto elemento di configurazione alla baseline, con la medesima versione. \todo{non so dove metterlo}

\subsubsection{Release management and delivery} \todo{non è riduttivo quello che c'è scritto? verificare se manca qualcosa, vedi standard ISO}
Dopo aver portato a termine le attività nel relativo, il responsabile del suo sviluppo è tenuto ad avviare una Pull Request per incorporare le modifiche nel ramo principale (main). La Pull Request viene approvata solo dopo che le modifiche sono state verificate. \todo{da riformulare, secondo me va scritto anche qualcosa di relativo alle release}

\paragraph{Questo dice ChatGPT}\todo{rimuovi}
Nello standard ISO/IEC 12207:1995, il processo di rilascio e consegna è definito come un processo che si occupa della pianificazione, del coordinamento e del controllo delle attività necessarie per preparare e distribuire una versione di un prodotto software per l'uso operativo. Questo processo comprende la gestione delle release, la distribuzione del software, la preparazione della documentazione correlata e altre attività correlate al rilascio e alla consegna del prodotto software.
...
\begin{itemize}
    ...
    \item \textbf{GitHub}: È una piattaforma web per il controllo di versione (tramite Git) dei Configuration Item e per il \hyperlink{par:ticketing}{Ticketing}. È impiegata per gestire le richieste di modifica tramite issue e label, oltre che per la contabilità dello stato di configurazione.
\end{itemize}

...
\begin{enumerate}
    ...
    \item \textbf{Pianificazione delle modifiche}: Se una change request viene approvata, viene pianificata e integrata nel ciclo di sviluppo del software. Questo può richiedere la rinegoziazione dei tempi di consegna, la revisione del piano di progetto, ecc; \todo{ecc...?}
...
\begin{itemize}
    \item \textbf{Registrazione delle configurazioni}: Registrazione delle informazioni dettagliate su ogni elemento di configurazione;
          \begin{itemize}
              \item  \textbf{Documentazione:} Le informazioni relative alla CI\todo{glossario?} sono presenti nella prima pagina di ciascuno.
              \item  \textbf{Sviulppo:} Le informazioni relative alla CI\todo{Configuration Item? correggere!} sono inserite come prime righe del file sotto forma di commento.
          \end{itemize}
    ...
    Ciò include le versioni attuali, le revisioni, le modifiche e le baselines;
          \begin{itemize}
              \item  \textbf{Registro delle modifiche:} Per monitorare lo stato di ciascun Configuration Item, si utilizza il registro delle modifiche incorporato in ognuno di essi; \todo{il registro delle modifiche è presente solo nei documenti}
              \item  \textbf{Branching \& DashBoard:} Per visionare la presenza di lavori in atto su di un Configuration Item si utilizza a supporto la presenza di branch attivi che lo riguardano e il posizionamento di issue associate nella Dashboard di progetto. \todo{da riformulare}
                    \begin{itemize}
...
\todo{parti sottostanti forse da mettere in introduzione all'inizio}

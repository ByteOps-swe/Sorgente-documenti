...
\begin{enumerate}
    ...
    \item \textbf{Stato: Redazione}
          \begin{enumerate}
              \item  Ogni tipo di documento viene creato secondo la struttura specificata nei prossimi paragrafi;\todo{riferimento preciso ai paragrafi}
...
\vspace{0.1cm}
\textbf{Caso redazione/modifica nuovo documento o sezione}\\
Dalla repository \href{https://github.com/ByteOps-swe/Sorgente-documenti}{Sorgente-documenti} contenente i sorgenti \LaTeX, il redattore dovrà creare un nuovo branch Git in locale e passare\todo{cambia} ad esso mediante l'utilizzo del comando:
\begin{lstlisting}[style=code]
    git checkout main
    git checkout -b branch_identificativoTask 
\end{lstlisting}

Il nome del branch destinato alla redazione o modifica del documento o di una sua sezione deve essere descrittivo al fine di consentire un'identificazione immediata e precisa del documento o della sezione su cui si sta lavorando. Pertanto, relativamente alla redazione dei documenti, devonono essere adottatate le \textbf{convenzioni per la nomenclatura dei branch} esposte all'interno della sezione 3.4.3.2. \todo{link}
...
\begin{enumerate}
    ...
    \item Attuano una \textbf{pull request:}
          \begin{enumerate}
              ...
              \item Eseguire i passaggi dettagliati nel caso "\textit{\textbf{Salvataggio e condivisone progressi di task non completate}}";\todo{link}
              \item Vai sul repository \todo{link} su GitHub >> Apri la sezione "pull request" >> Clicca "New pull request";
...
Corpo del documento: \todo{migliora struttura}
\begin{itemize}
    ...
    \item  \textbf{Attività da svolgere:} Tabella dove viene specificato:\todo{questo solo per gli interni}
          \begin{itemize}
...
Il nome dei documenti generati deve essere omogeneo, con la prima lettera maiuscola ed il resto minuscolo e, ad eccezione dei verbali, deve contenere un riferimento alla versione del documento (notazione di versionamento:~\ref{sec:RegistroModifiche}).\\ \todo{togli sta roba e scrivi solo la convenzione}
...
\begin{itemize}
    ...
    \item Viene inserito ';' alla fine delle voci dell'elenco tranne l'ultima che termina con '.' . Ogni voce dell'elenco inizia con una lettera maiuscola; \todo{punto e virgola, punto}
...
\begin{itemize}
    ...
    \item \textbf{Italico}: Riferimento a paragrafi, sezioni o documenti, nomi delle aziende e nomi propri dei membri del team.
\end{itemize}

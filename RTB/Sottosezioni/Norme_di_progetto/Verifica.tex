...
Lo scopo dell'\textit{attività}\textsubscript{\textit{G}} di verifica nello sviluppo \textit{software}\textsubscript{\textit{G}} è garantire la qualità del prodotto attraverso un'analisi accurata e sistematica. 
...
La verifica mira a garantire che il \textit{software}\textsubscript{\textit{G}} soddisfi gli \textit{standard}\textsubscript{\textit{G}} di qualità prestabiliti, riducendo il rischio di errori e migliorando l'affidabilità, la robustezza e l'efficienza del prodotto finale.
Questo processo verrà attuato per ciascuna \textit{attività}\textsubscript{\textit{G}} e dovrà assicurare che l'output di quest'ultima sia in uno stato stabile, consentendo così l'inizio della successiva fase di validazione. 

\subsubsection{Descrizione}
Il processo di verifica viene eseguito su tutti i \textit{processi}\textsubscript{\textit{G}} in esecuzione al raggiungimento di un livello di maturità adeguato o in seguito a modifiche dello stato del processo. Per i \textit{processi}\textsubscript{\textit{G}} di supporto, si analizza la qualità dei prodotti generati e dei \textit{processi}\textsubscript{\textit{G}} utilizzati al fine di garantire la conformità agli \textit{standard}\textsubscript{\textit{G}} di qualità definiti.
Le \textit{attività}\textsubscript{\textit{G}} relative al processo di verifica vengono assegnate ai verificatori, incaricati di analizzare i prodotti e valutarne la conformità ai vincoli qualitativi specificati nel Piano di Qualifica. Tali operazioni seguono l'ordine specificato nel modello a V.
Nel Piano di Qualifica sono documentate tutte le \textit{attività}\textsubscript{\textit{G}} che compongono il processo, descrivendone scopi, risultati sperati e risultati ottenuti. Questo fornisce linee guida per una corretta valutazione della qualità, normando e rendendo ripetibile il processo. 
...
Quando il verificatore individua un'Issue nella colonna "Da revisionare" della \href{https://github.com/orgs/ByteOps-swe/projects/1/views/1}{DashBoard documentazione}, sarà tenuto a convalidare il file corrispondente presente nella \textit{repository}\textsubscript{\textit{G}} "Sorgente documenti". \\
In aggiunta, il revisore riceverà una notifica via email quando il redattore completa la propria \textit{attività}\textsubscript{\textit{G}}, comunicandogli la presenza della \textit{pull request}\textsubscript{\textit{G}} assegnatagli. \\
Nella sezione "\textit{pull request}\textsubscript{\textit{G}}" di GitHub, il revisore troverà la richiesta di unire il \textit{branch}\textsubscript{\textit{G}} di redazione al \textit{branch}\textsubscript{\textit{G}} "main", assumendo il ruolo di revisore. Accedendo alla \textit{pull request}\textsubscript{\textit{G}} su GitHub, il revisore avrà la possibilità di esaminare attentamente il documento in questione e di aggiungere commenti visibili ai redattori nel caso in cui siano necessarie modifiche per la validazione. 
...
\begin{enumerate}
    \item \textbf{Accetta la Pull Request:} 
        accedere alla pagina della \textit{pull request}\textsubscript{\textit{G}} in cui si agirà con il ruolo di revisore nel \textit{repository}\textsubscript{\textit{G}} "Sorgente documenti" su GitHub >> Risolvere eventuali conflitti >> Merge Pull Request.
...
I walkthrough sono più adatti per \textit{attività}\textsubscript{\textit{G}} più creative o nelle fasi iniziali del progetto quando la flessibilità e la discussione sono importanti. Sono ideali quando si desidera una comprensione più approfondita e una valutazione qualitativa. 

Le \textit{attività}\textsubscript{\textit{G}} in cui verrà applicato Walkthrough sono:
\begin{itemize}
...
Queste liste fungono da \textit{checklist} per valutare l'accuratezza dell'\textit{attività}\textsubscript{\textit{G}} d'ispezione, determinando se è stata condotta in modo corretto.
...
Nel ciclo di sviluppo \textit{software}\textsubscript{\textit{G}}, è essenziale condurre una verifica accurata del codice prodotto al fine di garantirne il corretto funzionamento. Questo processo si avvale dell'analisi dinamica, una categoria di metodologie che richiedono l'esecuzione effettiva del prodotto. 
...
Per assicurare l'efficacia dell'analisi dinamica, è necessario automatizzare e rendere ripetibile questo processo, garantendo così una valutazione oggettiva del prodotto. Nel contesto dell'ingegneria del \textit{software}\textsubscript{\textit{G}}, il \textit{test}\textsubscript{\textit{G}} rappresenta la principale tecnica di analisi dinamica. 

\paragraph{Testing}
Il testing mira a garantire che la componente soggetta al TEST esegua in modo corretto le \textit{attività}\textsubscript{\textit{G}} assegnate e aderisca scrupolosamente ai vincoli ad essa attribuiti. Questi \textit{test}\textsubscript{\textit{G}} sono inoltre strumentali per individuare eventuali anomalie di funzionamento. 
...
\begin{itemize}
    ...
    \item \textbf{Input:} 
        i dati forniti in input per l'esecuzione del \textit{test}\textsubscript{\textit{G}};
    \item \textbf{Output:} 
...
Per l'implementazione di tali \textit{test}\textsubscript{\textit{G}} è concesso utilizzare oggetti simulati o parziali (mocks e stubs) al fine di separare l'unità di codice in esame dalle sue dipendenze esterne, permettendo così la verifica del suo comportamento in contesti controllati, con l'obiettivo di garantire un'isolamento efficace durante le fasi di \textit{test}\textsubscript{\textit{G}}. 
...
\begin{itemize}
    \item \textbf{Top-down:}
        l'\textit{integrazione}\textsubscript{\textit{G}} inizia con le componenti di \textit{sistema}\textsubscript{\textit{G}} che presentano maggiori dipendenze e un maggiore valore esterno, consentendo la tempestiva disponibilità delle funzionalità di alto livello.

        Ciò consente di effettuare \textit{test}\textsubscript{\textit{G}} prolungati sulle funzionalità principali, rendendole disponibili inizialmente ma richiede molti mock; 
...
\begin{itemize}
    \item \textbf{[tipo]:} 
        è il tipo di \textit{test}\textsubscript{\textit{G}};
        \begin{itemize}
            \item \textbf{U:} 
                per i \textit{test}\textsubscript{\textit{G}} di unità;
            \item \textbf{I:} 
                per i \textit{test}\textsubscript{\textit{G}} di \textit{integrazione}\textsubscript{\textit{G}}; 
            \item \textbf{S:} 
                per i \textit{test}\textsubscript{\textit{G}} di \textit{sistema}\textsubscript{\textit{G}}; 
            \item \textbf{R:} 
                per i \textit{test}\textsubscript{\textit{G}} di regressione;
            \item\textbf{A:} 
                per i \textit{test}\textsubscript{\textit{G}} di accettazione;
        \end{itemize}
    \item \textbf{[codice]:} 
...
Ad ogni \textit{test}\textsubscript{\textit{G}} verrà successivamente attribuito uno stato che rappresenterà il risultato dell'esecuzione. L'insieme dei risultati dei \textit{test}\textsubscript{\textit{G}} sarà registrato nel "cruscotto \textit{test}\textsubscript{\textit{G}}".

I \textit{test}\textsubscript{\textit{G}} nel cruscotto potranno assumere i seguenti stati: 

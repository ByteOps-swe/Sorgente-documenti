...
\paragraph{Scopo}
Il compito della Codifica, compiuto dal programmatore, mira a concretizzare il software richiesto dal proponente.
Quest'operazione trasforma le concezioni dei progettisti in codice, generando istruzioni eseguibili dai calcolatori.
I programmatori sono tenuti a osservare tali direttive nel corso delle fasi di programmazione e implementazione.
\paragraph{Aspettative}
La codifica è finalizzata alla creazione di un prodotto software in linea con le richieste del committente e conforme agli accordi stipulati. Il rispetto accurato delle norme stabilite assicura la produzione di un codice leggibile, uniforme e di alta qualità. Aderire alle norme e alle convenzioni è essenziale, poiché facilita le operazioni di estensione, manutenzione, verifica e validazione, contribuendo così al continuo miglioramento della qualità del prodotto.
\paragraph{Norme di codifica}
Il linguaggio adottato per svolgimento del progetto è python:  linguaggio di programmazione interpretato, ad alto livello e general-purpose.
Dal punto di vista tecnico, Python è dinamicamente tipizzato e dispone di un sistema di gestione della memoria automatico (garbage collection).
Per quanto riguarda il supporto ai thread, Python dispone di un modulo chiamato threading che consente la creazione e la gestione di thread.
Tuttavia, per il calcolo intensivo e la parallelizzazione su più processori, spesso si preferisce utilizzare il modulo multiprocessing che permette l'esecuzione di processi separati, superando le limitazioni del Global Interpreter Lock (GIL), che permette a un solo thread di eseguire istruzioni Python alla volta.
...
Per assicurare una strutturazione corretta del codice, è imperativo che i blocchi annidati adottino un'indentazione appropriata, con l'uso di un TAB per ciascun livello di annidamento.
Adottare un'omogeneità nella formattazione del codice al fine di agevolarne la comprensione e la gestione.
...
\begin{itemize}
    ...
    \item \textbf{Testabilità:}Metodi più brevi sono più facili da testare in isolamento, il che favorisce l'implementazione di test unitari efficaci;
...
\paragraph{Evita commenti inutili}
L'obiettivo è scrivere il codice in modo chiaro e autoesplicativo, riducendo la dipendenza da commenti esplicativi. 
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Usa nomi descrittivi per le immagini e i container.
        \item Mantieni una struttura coerente nei tuoi file Docker.
    \end{itemize}

\item \textbf{Versionamento:}
    \begin{itemize}
        \item Specifica sempre la versione dell'immagine di base (base image) per garantire la riproducibilità.
        \item Evita di usare tag "latest" per immagini di produzione.
    \end{itemize}

\item \textbf{Minimizzazione degli Strati (Layering):}
    \begin{itemize}
        \item Riduci il numero di istruzioni nell'esecuzione del Dockerfile per minimizzare gli strati dell'immagine.
        \item Raggruppa le istruzioni correlate per sfruttare la cache Docker.
    \end{itemize}

\item \textbf{Sicurezza:}
    \begin{itemize}
        \item Utilizza immagini ufficiali o verificate.
        \item Evita di eseguire processi Docker con privilegi elevati quando possibile.
        \item Usa ARG per parametrizzare informazioni sensibili.
    \end{itemize}

\item \textbf{Ottimizzazione delle Risorse:}
    \begin{itemize}
        \item Limita l'uso di risorse all'interno dei container (CPU, memoria, etc.).
        \item Usa immagini leggere e ottimizzate per la produzione.
    \end{itemize}

\item \textbf{Gestione delle Variabili d'Ambiente:}
    \begin{itemize}
        \item Usa variabili d'ambiente per configurazioni dinamiche.
        \item Fornisci valori predefiniti sensati per le variabili d'ambiente.
    \end{itemize}

\item \textbf{Logging e Monitoraggio:}
    \begin{itemize}
        \item Configura i container per registrare i log in modo efficace.
        \item Integra strumenti di monitoraggio, se necessario.
    \end{itemize}

\item \textbf{Pulizia e Riduzione delle Dimensioni:}
    \begin{itemize}
        \item Pulisci i pacchetti temporanei e le risorse non necessarie dopo l'installazione delle dipendenze.
        \item Riduci le dimensioni delle immagini utilizzando multi-stage builds.
    \end{itemize}

\item \textbf{Documentazione:}
    \begin{itemize}
        \item Aggiungi commenti nel Dockerfile per spiegare le decisioni di progettazione.
        \item Fornisci una documentazione chiara su come utilizzare e configurare il container.
    \end{itemize}

\item \textbf{Testing:}
    \begin{itemize}
        \item Implementa test automatizzati per il tuo Dockerfile e i tuoi container, se possibile.
    \end{itemize}
\end{itemize}
\paragraph{Strumenti}
\begin{itemize}
...
\begin{itemize}
    ...
    \item Struttura del prodotto software.
\end{itemize}
...
I diagrammi dei casi d'uso sono uno strumento di modellazione utilizzato nell'ingegneria del software per rappresentare le interazioni tra un sistema e gli attori (utenti o altri sistemi) che interagiscono con esso. Ecco una spiegazione generale dei principali elementi di un diagramma dei casi d'uso:

\begin{enumerate}
  \item \textbf{Attori:}
    Gli attori sono le entità esterne al sistema che interagiscono con esso. Possono essere utenti umani, altri sistemi software o componenti esterne. Gli attori sono rappresentati come "stickman" all'esterno del rettangolo del sistema.
    ...
    \todo{Non capisco perché l'immagine diventa così grande e sgranata, forse ho sbagliato package}

  \item \textbf{Casi d'Uso:}
    I casi d'uso rappresentano le diverse interazioni che un attore può avere con il sistema. Ogni caso d'uso è rappresentato da un ovale all'interno del rettangolo del sistema e contiene l'identificativo e il titolo esplicativo del tipo di interazione che si verifica.
    \begin{minipage}[t]{\linewidth}
        \centering
  ...
  \item \textbf{Relazioni tra Attori e Casi d'Uso:}
    Le linee di associazione collegano gli attori ai casi d'uso corrispondenti, indicando quali attori sono coinvolti in una particolare interazione. Ad esempio, una linea collega un attore a un caso d'uso se quell'attore è coinvolto in quell'interazione.
    ...
    \todo{metto il riquadro del sistema come nelle figure sotto o lascio stare come nelle figure sopra?}

  \item \textbf{Estensioni e Inclusioni:}
  \begin{enumerate}
  ...
  \item \textbf{Sistema:}
    Il rettangolo esterno rappresenta il sistema stesso. All'interno di questo rettangolo sono posizionati attori e casi d'uso.
\end{enumerate}

\todo{Mancano sottocasi d'uso e generalizzazione tra attori, non so se è voluto}
...
Il flusso di lavoro adottato per la documentazione, denominato ‘feature branch workflow’, prevede la creazione di un branch dedicato per ogni documento o sezione richiesto.
...
L'inclusione di CI nel branch principale (main) lo configura come la versione più recente, revisionata, mentre le versioni (X.0.0) con X>=1 identificano il CI facente parte della baseline con uguale versione.% -----------------------------Processi organizzativi --------------------------------

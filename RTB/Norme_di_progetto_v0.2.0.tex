...
\begin{itemize}
    \item Comprensione del prodotto senza supporto umano, usandolo come mezzo di comunicazione;
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Verbali:}Contiene tutti i verbali redatti durante il periodo di candidatura, contiene una Suddivisione tra verbali esterni ed interni;
    ...
    \begin{itemize}
        \item \textbf{Verbali:}Contiene tutti i verbali redatti durante il periodo RTB, contiene una Suddivisione tra verbali esterni ed interni;
...
\begin{itemize}
    ...
    \begin{itemize}
        \item  \textbf{Registro delle modifiche:} Per tenere traccia dello stato di ogni CI si adotta il registro delle modifiche presente ognuno di questi;
        ...
        \begin{itemize}
            \item Ogni issue è associato ad un configuration item tramite label. (ex. Issue relative a Norme di progetto avranno tra i label: NormeDiProgetto).
        \end{itemize} 
    \end{itemize}
    ...
    \end{itemize}
\end{itemize}
La Contabilità dello Stato di Configurazione processo fondamentale per mantenere la trasparenza e la tracciabilità nel ciclo di vita del software, aiutando a gestire le configurazioni in modo coerente e a mantenere un registro accurato di tutte le attività e le modifiche che coinvolgono gli elementi di configurazione.\\
\vspace{0.2cm}
(Ex. l'inclusione di un CI nel branch principale (main) lo configura come la versione più recente, revisionata, mentre le versioni (X.0.0) con X>=1 identificano il CI facente parte della baseline con uguale versione.)

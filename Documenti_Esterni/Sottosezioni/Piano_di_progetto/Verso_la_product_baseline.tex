...
\paragraph{Considerazioni}
Durante il nono periodo, sono stati dedicati notevoli investimenti di risorse alle attività di progettazione, sviluppo e testing relative al Minimum Viable Product (MVP). I progettisti e gli sviluppatori hanno completato le attività di progettazione e sviluppo delle dashboard in Grafana. Questo ha portato alla realizzazione di una dashboard principale, concepita per offrire una visione d'insieme dello stato di salute della città, e una dashboard secondaria, pensata per consentire un'analisi dettagliata delle misurazioni mediante l'applicazione di filtri personalizzati. \\
In seguito al completamento delle dashboard, è stato avviato il processo di redazione del Manuale Utente. Tuttavia, va notato che non è stata pianificata alcuna attività di verifica in questa fase; tale verifica sarà programmata per il periodo successivo. \\
Progettisti e programmatori, oltre alla realizzazione delle dashboard, hanno portato a termine la progettazione e lo sviluppo del database e si sono dedicati all'implementazione della funzionalità di calcolo del punteggio di salute tramite la libreria Faust. \\
Nel documento "\textit{Specifica Tecnica}", sono state completate le sezioni introduttive e relative alle tecnologie utilizzate ed è stata avviata la sezione relativa all'architettura del database. \\
L'amministratore si è dedicato alla modifica della struttura della repository, in risposta alle osservazioni ricevute dal Prof. Vardanega durante la revisione RTB. \\
Parallelamente allo sviluppo delle componenti principali del sistema, sono stati avviati i test, con particolare attenzione allo sviluppo dei test di unità per i simulatori dei sensori e all'avvio dei test di integrazione tra Python e Kafka e tra Python e Clickhouse.

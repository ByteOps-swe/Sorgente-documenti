...
\paragraph{Considerazioni}
Durante il decimo periodo, il team si è dedicato principalmente allo sviluppo dei test e all'implementazione delle ultime funzionalità in vista della presentazione dell'MVP. \\
L'Amministratore ha apportato importanti miglioramenti alla pagina GitHub.io, rendendo la presentazione dei documenti e degli artefatti del progetto più intuitiva, seguendo i consigli del Prof. Vardanega emersi durante la revisione RTB. Inoltre, si è occupato dell'automatizzazione dei test configurando la pipeline CI per garantire la solidità del processo di integrazione continua. \\
Il team di progettisti e programmatori ha collaborato per configurare e implementare la funzionalità Time to Live (TTL) nel database Clickhouse. Questa funzionalità consente di risparmiare notevolmente spazio di archiviazione, eliminando automaticamente le misurazioni precedenti a un periodo stabilito. Nel nostro caso, questo periodo è di un mese. Tuttavia, anziché eliminare direttamente le misurazioni, prima della loro rimozione definitiva, viene calcolata una media di tali misurazioni per ogni ora. In questo modo, invece di mantenere in memoria tutte le misurazioni meno recenti, viene conservata una sola misurazione aggregata per ogni intervallo orario. Questo approccio assicura un significativo risparmio di spazio di archiviazione nel database, garantendo al contempo la preservazione di una sintesi delle misurazioni meno recenti, la quale consente l'analisi dei dati anche relativi a periodi molto remoti. \\
I progettisti hanno anche completato la sezione relativa al database nel documento \textit{Specifica Tecnica} e hanno lavorato attivamente alla progettazione dei test di unità, di integrazione e di sistema per garantire la solidità e l'affidabilità del prodotto. \\
I programmatori hanno implementato filtri per gestire le misurazioni corrotte, migliorando così la stabilità e l'integrità dei dati. Inoltre, hanno aggiunto l'indicazione dell'unità di misura nei grafici sulla dashboard e apportato miglioramenti grafici per ottimizzare l'esperienza utente. Inoltre, è stato sviluppato un sistema per ricevere le notifiche di allerta su Discord ed è stato completato il \textit{Manuale Utente}. \\
I verificatori hanno svolto un ruolo cruciale nella garanzia della qualità del software, sviluppando test sul calcolo del punteggio di salute, test di integrazione per verificare l'integrità del flusso dei dati tra Python e Kafka, test sulla gestione dei malfunzionamenti e test di carico e performance. Si sono anche occupati della verifica del \textit{Manuale Utente} per garantirne chiarezza e usabilità per gli utenti finali. \\
Infine, il Responsabile ha coordinato le attività del team, assicurandosi che il lavoro svolto fosse in linea con gli obiettivi prefissati. Ha redatto il resoconto del periodo e ha validato il Manuale Utente per assicurare il rispetto degli standard di qualità richiesti.
...
\begin{itemize}
    \item \textbf{Rischi attesi e verificati:}
        \begin{itemize}
            \item \textbf{RT-1M-4} - Basse prestazioni hardware per i test di performance (\textit{\ref{sec:lowPrestazioniHW}});
            \begin{itemize}
                \item \textbf{Esito mitigazione}: Data la mancanza di risorse per mettere su un ambiente distribuito e i limiti delle nostre macchine per eseguire test di carico e performance, non siamo stati in grado di attuare alcuna forma di mitigazione. Abbiamo semplicemente dovuto fare affidamento sulle risorse disponibili, pur riconoscendo che queste possono non essere sufficienti per condurre test esaustivi.
                \item \textbf{Impatto}: L'impossibilità di mitigare il rischio ha avuto un impatto diretto sulla nostra capacità di valutare accuratamente le prestazioni dell'applicativo. I test condotti potrebbero non essere stati rappresentativi delle condizioni reali di utilizzo e potrebbero non aver identificato problemi di prestazioni cruciali. Dobbiamo essere consapevoli di questa limitazione mentre procediamo nello sviluppo del progetto e considerare alternative per compensare questa mancanza di risorse, come l'ottimizzazione del codice o l'uso di strumenti di simulazione più leggeri.
            \end{itemize}
            \item \textbf{RP-3M-2} - Contatti con la proponente (\textit{\ref{subsubsec:contattiProponente}})
                \begin{itemize}
                    \item \textbf{Esito mitigazione}: Nonostante la partenza in ferie del membro più specializzato dell'azienda proponente, il rischio non ha avuto un impatto significativo sul nostro progresso. Essendo stati avvisati in anticipo, come team ci siamo preparati a questa eventualità. Prima della sua partenza, abbiamo lavorato intensamente per risolvere la maggior parte dei dubbi e delle questioni aperte. Inoltre, abbiamo pianificato la distribuzione dei compiti in modo che i membri rimanenti potessero gestire efficacemente il lavoro durante la sua assenza.
                    \item \textbf{Impatto}: Nonostante la temporanea assenza del membro chiave, siamo stati in grado di mantenere un buon ritmo di lavoro e di risolvere i problemi in modo tempestivo. Tuttavia, è importante tenere presente che la sua assenza potrebbe comunque aver rallentato il processo decisionale in alcune aree, e che alcuni dubbi potrebbero emergere durante la sua assenza, richiedendo una risoluzione successiva. Ad ogni modo, abbiamo dimostrato una buona capacità di adattamento e gestione delle risorse interne per affrontare questa eventualità.
                \end{itemize}
        \end{itemize}
    \item \textbf{Rischi attesi ma non verificati:} 
        \begin{itemize}
            \item \textbf{RO-2A-4} - Inesperienza nell'attività di testing (\textit{\ref{sec:inexpAttività}}) \\
            Durante la pianificazione delle attività dell'attività di testing, è stato considerato il rischio di rallentamenti legato all'inesperienza del team in questo specifico ambito. Tuttavia, questo rischio non si è concretizzato in quanto la formazione e l'esperienza accumulata durante i periodi precedenti si sono rivelate sufficienti per affrontare con successo le sfide di testing durante l'attuale periodo.
        \end{itemize}
    \item \textbf{Rischi non attesi ma verificati:}
        \begin{itemize}
            \item Nessuno
